       ...
       return (
            <div className="pb-12">
                ...
                <div className="flex justify-between items-center mb-6">
                   <div className="flex gap-4 border-b border-white/10 pb-4 flex-1">
                        <button 
                            onClick={() => setActiveTab('my-stacks')}
                            className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'my-stacks' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}
                        >
                            My Stacks
                            {activeTab === 'my-stacks' && <m.div layoutId="activeTabStack" className="absolute bottom-[-17px] left-0 right-0 h-1 bg-[#FF6B00] rounded-t-full" />}
                        </button>
                        <button 
                            onClick={() => setActiveTab('community')}
                            className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'community' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}
                        >
                            Community Explore
                             {activeTab === 'community' && <m.div layoutId="activeTabStack" className="absolute bottom-[-17px] left-0 right-0 h-1 bg-[#FF6B00] rounded-t-full" />}
                        </button>
                    </div>
                    ...
                    <div className="flex gap-3">
                         <button 
                            ...
                            className={`px-3 py-2 rounded-lg text-xs font-bold transition-all border flex items-center gap-2 ${showGuide ? 'bg-white/10 text-white border-white/20' : 'bg-white/5 text-gray-200 border-white/20 hover:bg-white/10 hover:border-white/30 hover:text-white'}`}
                        >
                            <FaQuestionCircle size={14} /> <span className="hidden sm:inline">{showGuide ? 'Hide Guide' : 'Guide'}</span>
                        </button>
                        <button 
                            onClick={() => setCreateMode(true)}
                            className="px-4 py-2 bg-white text-black font-bold rounded-lg hover:bg-gray-200 transition-colors flex items-center gap-2 text-xs shadow-[0_0_10px_rgba(255,255,255,0.1)]"
                        >
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/5">
                        ...
                        {activeTab === 'my-stacks' && (
                            ...
                            >
                                <Plus className="inline mr-2" size={16}/> Create Stack
                            </button>
                        )}
                    </div>
                ) : (
