...
const Home = ({ isCinematicLoading }) => {
    ...
    // --- Infinite Marquee Featured Tools - Removed unused ---
    // Modal state for detailed tool view
    const [selectedTool, setSelectedTool] = useState(null);

    // openModal removed (unused)
    ...
    // suggestions and activeSuggestionIndex removed (unused)
    const [isMobile, setIsMobile] = useState(() => typeof window !== 'undefined' ? window.innerWidth < 768 : false);
    // mobileSheetOpen unused
    ...
    // mobileSheetOpen useEffect removed
    ...
    // mobilePlaceholderTexts removed
    // desktopPlaceholderTexts removed
    // placeholderFade, currentPlaceholderText unused logic removed


    // Placeholder rotation logic removed (unused variables)
    ...
    // isLoggedIn, username, getGreeting removed
        ...
        const staticCategoriesMap = new Map(
            toolsData.map(cat => ({
                ...
                tools: cat.tools.filter(t => 
                    !backendSignatures.has(normalize(t.url)) && 
                    !backendSignatures.has(normalize(t.name))
                )
                .map(t => ({
                        ...t,
        ...
        convertedApprovedTools.forEach(tool => {
            if (staticCategoriesMap.has(tool.category)) {
                // Check uniqueness before adding
                 // Backend URLs are sometimes just 'example.com', static are 'https://example.com'
                 // But we already filtered static.
                 // We should be careful not to add same tool twice if it came from backend?
                 // convertedApprovedTools is unique by ID usually.
    ...
    // Helper for "isAddedToday" logic
    const isAddedToday = useCallback((date) => {
        if (!date) return false;
    ...
    const toolsAddedTodayCount = useMemo(() => {
        return toolList.filter(t => isAddedToday(t.dateAdded)).length;
    }, [toolList, isAddedToday]);
    ...
    const handleInputChange = (e) => {
        ...
        // Auto-scroll to tools on mobile when typing - DISABLED per user request
        /* 
        ...
        */
        ...
        // setActiveSuggestionIndex and setSuggestions logic removed (unused)
        clearTimeout(debounceRef.current);
        debounceRef.current = setTimeout(() => {
            // Logic reserved for future search enhancements
        }, 150);
    };
    ...
    const filteredTools = useMemo(() => {
        ...
        result = result.map(category => ({
            ...
            tools: [...category.tools].sort((a, b) => {
                ...
                if (sortOrder === 'date_asc') return (a.dateAdded || 0) - (b.dateAdded || 0);
                
                // fallback or default: 'date'/'date_desc' (Newest First)
    ...
    }, [searchQuery, activeFilter, activePricing, sortOrder, isMobile, history]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    // trendingTools and newTools removed
    ...
    const { visibleCategories, hiddenCategories } = useMemo(() => {
        ...
        return {
            ...
            hiddenCategories: all.slice(10).sort((a, b) => a.label.localeCompare(b.label))
        };
    }, [categories]);
                                            ...
                                            id === 'website-builders' ? 'Website Builders' :
                                                id === 'image-generators' ? 'Image Generators' :
                                                    id === 'email-assistance' ? 'Email Assistance' :
                                                        ...
                                                        // categoriesToShow removed
                                                                                                        id === 'choice' ? 'Admin Choice' :
                                                                                                            id.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
    );

    // userCats and dataCats removed
    // allSet calculation removed
    // allSet unused after extra removal, but kept if needed for logic below?
    // User says allSet is unused. Checking logic...
    // If unused, remove.
    // const extra = allSet.filter(id => !curatedBase.includes(id));
    // extra removed (unused)
    // collapsed and expanded removed
    // categoriesToShow removed
        ...
        <>
            <LazyMotion features={domAnimation}>
                ...
                <div className="min-h-screen text-white relative">
                                ...
                                <div className="space-y-6">
                                     <div className="group text-right">
                                        ...
                                        <div className="flex items-center justify-end gap-2 mt-1">
                                            ...
                                            <p className="text-sm text-gray-500 font-medium uppercase tracking-widest">AI Tools</p>
                                        </div>
                                     </div>
                                     ...
                                     <div className="group text-right">
                                        ...
                                        <div className="flex items-center justify-end gap-2 mt-1">
                                            <p className="text-sm text-gray-500 font-medium uppercase tracking-widest">Categories</p>
                                        </div>
                                     </div>
                                </div>
                            </div>
                    ...
                    {/* Tool of the Day */}
                    {!isMobileSearchActive && <ToolOfTheDay />}
                            ...
                            <div className="bg-[#050505]/95 backdrop-blur-2xl border-b border-white/10 shadow-2xl">
                                <div className="max-w-7xl mx-auto px-4 pt-4 pb-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <SurpriseMeButton tools={toolList} className="hidden md:flex" />
                    ...
                    {/* Mobile Top Nav - Logo + Categories/Picks (Mobile Only) */}
                    {
                        isMobile && (
                            <MobileTopNav
                                ...
                                onSearchOpenChange={setIsMobileSearchActive}
                                tools={toolList}
                                onCategorySelect={(id) => {
                                    setActiveFilter(id);
                                    setTimeout(() => {
                            ...
                            <main className="w-full">
                                ...
                                >
                                    ...
                                    <div className="w-full relative flex flex-col lg:flex-row gap-8">
                                        <aside className={`w-full lg:w-48 xl:w-64 flex-shrink-0 space-y-8 z-30 self-start ${isMobile ? (isMobileSearchActive ? 'hidden' : 'mb-6') : 'lg:sticky lg:top-32'}`}>
                                            {/* Header Group */}
                ...
                {/* Testimonials / Moving Cards - Full Width Footer Style */}
